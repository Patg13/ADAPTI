...
struct thread_data
{
	...
	int p_maxoffset;
	int p_ambigmax;
	vector<string>* fprimer;
	vector<string>* rprimer;
	bool p_ignore_reverse;
	//bool p_mirror_primers;
};
...
void *TrimSequences_Thread(void *threadarg)
{
	...
	for (iter=my_data->p_dSeq->begin(); iter != my_data->p_dSeq->end(); ++iter)
	{
		...
		*/
		...
		for (piter=my_data->fprimer->begin(); piter != my_data->fprimer->end();++piter){
			posFor=FindTrimPosition((*iter)->reqSeq(), (*piter), my_data->p_mismatch, my_data->p_maxoffset);
			if (posFor != 0){
				break;
			}
		}
		...
		if ( my_data->p_ignore_reverse == false ){
			for (piter=my_data->rprimer->begin(); piter != my_data->rprimer->end(); ++piter){
				posRevc=FindTrimPosition(ReverseComplementarySeq((*iter)->reqSeq()), (*piter), my_data->p_mismatch, my_data->p_maxoffset);
				if (posRevc != 0){
					break;
				}
			}
...
int main(int argc, char** argv)
{
	...
	string version="5.0";
	try{
		TCLAP::CmdLine cmd("This program trim the forward and the reverse sequencing primers and everything before and after", ' ', version);
	...
	// Option Parsing
		...
		vector<string> rprimer_vect;
		 ...
		 //cout << fprimer_is_file << " " << rprimer_is_file << endl;
		 ...
		 if ( fprimer_is_file == true){
			 if ( ConvertFile2Vector(For_Primer,&fprimer_vect) == false ){
				 cerr << "ERROR: One or more forward primers is/are not valid in the file" << endl;
				 return -1;
			 }
		 }else{
			 if ( IsValidPrimer(For_Primer) == false){
				 cerr << "ERROR: Forward Primer not valid or file not found : " << For_Primer << endl;
				 return -1;
			 }
			 fprimer_vect.push_back(For_Primer);
		 }

		 if ( rprimer_is_file == true){
			 if ( ConvertFile2Vector(Rev_Primer,&rprimer_vect) == false ){
				 cerr << "ERROR: One or more reverse primers is/are  not valid in the file" << endl;
				 return -1;
			 }
		 }else{
			 if (ignore_reverse == false ){
				 if ( IsValidPrimer(Rev_Primer) == false){
					 cerr << "ERROR: Reverse Primer not valid or file not found : " << Rev_Primer << endl;
					 return -1;
				 }
				 rprimer_vect.push_back(Rev_Primer);
			 }
		 }
		...
		if (display_debug)
		{
			...
			if (fprimer_is_file == true){
				cout << "Forward primer is a file" << endl;
			}
			if (rprimer_is_file == true){
				cout << "Reverse primer is a file" << endl;
			}
	...
	/*
		...
		for (i=0; i < num_threads; i++)
		{
			...
			td[i].fprimer=&fprimer_vect;
			td[i].rprimer=&rprimer_vect;
			td[i].p_ignore_reverse=ignore_reverse;
			//td[i].p_mirror_primers=mirror_primers;
