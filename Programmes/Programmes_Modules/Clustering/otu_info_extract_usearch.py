...
print("usearch otu table ---> frequencies table maker V2 for usearch clustering")
print("By: Patrick Gagne")



parser=argparse.ArgumentParser(description='usearch otu table information extractor V2 for usearch')

parser.add_argument("-up", dest="uparse_file", required=True, help=("uparse table produced by option -uparseout [REQUIRED]"))
...
#args=parser.parse_args('-up otu.up -group myriam.group -name myriam.trimmed.names -out result.table'.split())
args=parser.parse_args() 

file=open(str(args.uparse_file),'r')
...
for i in fileL:
    if i.split("\t")[1]=="otu":
        seq_list=[]
        otu_count+=1
        otu_list.append("OTU_"+str(otu_count))
        seq_list.append(i.split(";size=")[0])
        otu_corresp["OTU_"+str(otu_count)]=seq_list
