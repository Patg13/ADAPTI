...
print("ITS1 Sequences Completion Program (for usearch/uparse based clustering)")
...
parser.add_argument("-uparse", dest="uparse_file", help=("Uparse table containing OTUs --- Sequences correspondances in usearch format [REQUIRED]"))
...
#args=parser.parse_args('-otus usearch.otus.ITS1.fasta -uparse usearch.otu.up -orifas usearch.desized.NO_HOMOP.fasta -out usearch.result.fasta'.split())
...
for i in uparse_otu_lines:
    spl=i.split("\t")
    otu_nb=spl[-1]
    upseqname=spl[0].split(";size=")[0]
    ori_seq=seq_dict[upseqname]
    otus_dict[otu_nb]=ori_seq
